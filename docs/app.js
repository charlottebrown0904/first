...
/* 모든 계산은 backend.js 가 브라우저 안에서 합니다. 그래야 GitHub Pages 처럼
   서버가 없는 곳에서도 파라미터를 바꿔가며 백테스트할 수 있습니다. */
const api = (path, params = {}) => Backend.call(path, params);
...
$('#rawCsv').addEventListener('click', () =>
  Backend.downloadCsv($('#rawStart').value || null, $('#rawEnd').value || null)
    .catch(showErr));
...
$('#btnRefresh').addEventListener('click', async () => {
  ...
  try {
    ...
    b.disabled = false; b.textContent = '시세 새로 받기';
    alert('시세 갱신은 로컬에서 python app.py 로 띄웠을 때만 됩니다. '
        + '이 페이지는 정적으로 배포된 사본이라 서버가 없습니다.');
  }
});
...
/* The refresh button needs a local Python process behind the page; on the
   published copy there isn't one, so ask first and hide it if absent. */
